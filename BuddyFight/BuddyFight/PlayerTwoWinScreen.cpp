...
void PlayerTwoWinScreen::InitializeScene()
{
	...
	// logo entities    
	winLogo = new Entity(new Texture("WinLogo.png", 0, 0, 178, 54), Graphics::SCREEN_WIDTH * 0.42f, Graphics::SCREEN_HEIGHT * 0.32f);
	winLogo->SetScale(V2ONE * 2);
	pool->AddEntity(winLogo);

	// play mode entities 
	playerTwo = new Entity(new Texture("PLAYER TWO", "emulogic.TTF", 32, { 230, 230, 230 }), Graphics::SCREEN_WIDTH * 0.5f, Graphics::SCREEN_HEIGHT * 0.22f);
	pool->AddEntity(playerTwo);

	restart = new Entity(new Texture("PRESS ENTER TO RESTART OR ESCAPE FOR STARTSCREEN", "emulogic.TTF", 32, { 230, 230, 230 }), Graphics::SCREEN_WIDTH * 0.5f, Graphics::SCREEN_HEIGHT * 0.8f);
